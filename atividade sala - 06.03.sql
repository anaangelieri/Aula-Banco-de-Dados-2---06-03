...
alter table Cliente modify column Email varchar(305) not null;
...
select * from cliente where salario is null;

update cliente set salario = 0 where Email like 'maria@gmail.com' and salario is null;

#Listar o Cliente com o menor Salario conhecido e diferente de 0 
select * from Cliente where Salario is not null  and Salario > 0;

select * from Cliente where Salario = (select min(Salario) from Cliente where Salario is not null and Salario > 0);

# Consultar dados do cliente com cartão de numero: 1234 5678 9012 3456
select * from Cliente where ClienteID = (select ClienteID from CartaoCredito where NumeroCartao = '1234 5678 9012 3456');
#select ClienteID, CartaoID from CartaoCredito where NumeroCartao = '1234 5678 9012 3456'

#Listar todos os clientes com conta do tipo 'Corrente'
select * from ContaBancaria where TipoConta = 'Corrente';

#Listar todos os cliente com limite no cartão maior que 500.00
select * from CartaoCredito where LimiteCredito > 500.00;

#Conta Corrente + Limite Cartão de Credito > 500.00
select * from Cliente where ClienteID in
	(select ClienteID from ContaBancaria where TipoConta = 'Corrente' and ClienteID in (select ClienteID from cartaocredito where LimiteCredito > 200.00));
